...
class PLLWC_Admin_Products {
			...
			if ( is_a( $tr_product, 'WC_Product_Variable' ) ) {
				...
				if ( ! empty( $tr_attributes ) ) {
					...
					foreach ( $tr_product->get_children() as $_tr_id ) {
						...
						if ( $attributes === $tr_variation->get_attributes() ) {
	...
	public function delete_variation( $id ) {
		...
		if ( ! in_array( $id, $avoid_delete ) ) {
			...
			foreach ( $tr_ids as $tr_id ) {
				$variation = wc_get_product( $tr_id );
	 ...
	 */
	static public function should_copy_texts( $from, $to, $sync ) {
		if ( ! $sync ) {
		...
		if ( isset( PLL()->sync_post ) ) {
			...
			if ( ! empty( $from ) && ! empty( $to ) ) {
				if ( 'variation' === $from->get_type() ) {
					return PLL()->sync_post->are_synchronized( $from->get_parent_id(), $to->get_parent_id() );
				} else {
					return PLL()->sync_post->are_synchronized( $from->get_id(), $to->get_id() );
				}
			}
		}
	 ...
	 */
	static public function maybe_translate_property( $value, $prop, $lang ) {
		switch ( $prop ) {
			case 'image_id':
	...
	public function search_found_products( $products ) {
		...
		if ( ! isset( $_REQUEST['pll_post_id'] ) || ! $lang = $this->data_store->get_language( (int) $_REQUEST['pll_post_id'] ) ) {
