...
class PLLWC_Links {
	...
	public function prepare_rewrite_rules( $pre ) {
		if ( ! has_filter( 'rewrite_rules_array', array( $this, 'rewrite_shop_rules' ) ) ) {
			add_filter( 'rewrite_rules_array', array( $this, 'rewrite_shop_rules' ) );
	...
	public function pll_translation_url( $url, $lang ) {
		...
		if ( is_shop() && ! is_search() ) {
			...
			foreach ( wc_get_attribute_taxonomies() as $tax ) {
				...
				if ( ! empty( $_GET[ $name ] ) && $tr_id = pll_get_term( (int) $_GET[ $name ], $lang ) ) {
	 ...
	 * @return array
	 */
	static public function set_home_urls( $languages ) {
	 ...
	 * @return array
	 */
	static public function pll_after_languages_cache( $languages ) {
